...
const ContactDetail = () => {
    ...
    const handleDeleteAddress = async (addressId: string) => {
        const confirm = await alertConfirm('Are you sure you want to delete this address?');
        if (!confirm) return;
        ...
        if (response.status === 200) {
            await alertConfirm('Address deleted successfully');
            await fetchAddress();
        } else {
            await alertError(responseBody.errors);
        }
                        ...
                        <div className="space-y-5 mb-8">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                {["first_name", "last_name"].map(field => (
                                    <div key={field} className="bg-gray-700 bg-opacity-50 p-5 rounded-lg shadow-md border border-gray-600 hover:bg-opacity-70">
                                        <div className="flex items-center mb-2">
                                            <i className="fas fa-user-tag text-blue-400 mr-2" />
                                            <h3 className="text-gray-300 text-sm font-medium">{field === "first_name" ? "First Name" : "Last Name"}</h3>
                                        </div>
                            ...
                            {["email", "phone"].map((field, i) => (
                                <div key={field} className="bg-gray-700 bg-opacity-50 p-5 rounded-lg shadow-md border border-gray-600 hover:bg-opacity-70">
                                    <div className="flex items-center mb-2">
                                        <i className={`fas ${i === 0 ? "fa-envelope" : "fa-phone"} text-blue-400 mr-2`} />
                                        <h3 className="text-gray-300 text-sm font-medium">{field === "email" ? "Email" : "Phone"}</h3>
                                    </div>
                                    <p className="text-white text-lg ml-6">{contact[field]}</p>
                                </div>
                            ))}
                                    ...
                                    <div key={address.id} className="bg-gray-700 bg-opacity-50 p-5 rounded-lg shadow-md border border-gray-600 card-hover">
                                        ...
                                        <div className="space-y-3 text-gray-300 ml-2 mb-4">
                                            {[
                                                ...
                                                { label: "Province", value: address.province, icon: "fa-map" },
                                                { label: "Country", value: address.country, icon: "fa-flag" },
                                                { label: "Postal Code", value: address.postal_code, icon: "fa-mailbox" },
