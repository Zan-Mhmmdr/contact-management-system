...
const ContactList = () => {
  ...
  const fetchContacts = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setTotalPages(data.paging?.total_page || 1);
      } else {
        alertError(data.errors || "Failed to fetch contacts.");
      }
    } catch (error) {
      console.error("Error fetching contacts:", error);
      alertError("An unexpected error occurred while fetching contacts.");
    }
  };
  ...
  const handleContactDelete = async (contactId: number) => {
    ...
    if (response.status === 200) {
      ...
      await alertError(
        responseBody.errors || "Failed to delete contact. Please try again."
      );
    }
  };
